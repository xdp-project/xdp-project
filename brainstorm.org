...
in files =test_xdp_vlan.c= and =test_xdp_vlan.sh=.



The ip utility (from iproute2) support specifying, that an
XDP prog must use generic XDP when loading an XDP prog.
