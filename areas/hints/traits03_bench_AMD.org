...
UPDATE: This IBPB test might be INVALID as it was tested without:
 - [[https://git.kernel.org/pub/scm/linux/kernel/git/torvalds/linux.git/commit/?id=ade8ff3b6aca47c234e][Merge tag 'x86_bugs_post_ibpb']]
...
| AMD EPYC 9684X     | cycles | cycles | nanosec | nanosec |
|--------------------+--------+--------+---------+---------|
| function call      |      3 |      3 |   1.356 |   1.356 |
| indirect call      |     15 |     15 |   6.237 |   6.235 |
| bpf_xdp_trait_set  |    111 |    111 |  43.925 |  43.742 |
| bpf_xdp_trait_get  |     21 |     21 |   8.627 |   8.373 |

It is a very *positive surprise* that (for this =kernel->kernel= use-case) the
IBPB mode gives same performance as *off* mode. We were expecting high
