...
Could we add this as a BTF field?

OK to reject at load time - user's problem to make sure the right feature
flags are set on the calling program for tail calls.
...
- Page pool -> driver support
- TX resource -> better XDP redirect support
