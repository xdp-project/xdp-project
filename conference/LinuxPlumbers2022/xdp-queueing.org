...
* Somewhere to store the packets                                     :export:
#+begin_quote
...
** Data structures for packet queues - prior art                    :export:
#+begin_src sh
...
- We don't need to limit ourselves to the PIFO, *however*:
  - We can use an /optimised implementation/ for software by Saeed et al^{=[1]=}
...
** The Eiffel PIFO implementation                                   :export:
:PROPERTIES:
:reveal_extra_attr: class="img-slide"
:END:
...
- The Eiffel PIFO implementation /performs well/
