...
- target audience is a kernel developer that wants to extend an existing XDP
  capable driver to support /non-linear buffers/
...
- XDP supported capabilities for /XDP_REDIRECT/
- XDP return path
#+begin_src C
