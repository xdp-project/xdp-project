...
- Trigger to enable, when (1) =ndo_bpf= attach SKB-update BPF-prog
- and/or when (2) =ndo_bpf= query to get BTF-format (maybe map-fd)

*Idea*: =ndo_bpf= /query return metadata-map/ (with associated *BTF-format*)
- Map is used by XDP-prog being attached (check BTF-format match)
  - then map-refcnt, determine when to disable populate metadata again
