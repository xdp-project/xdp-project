...
- /Point#B/: Driver function *populates metadata with offloads* from descriptor
...
- /Point#A/: Driver defines static info-struct for metadata area
  - Create BTF-format (via macros) and register with BPF
- /Point#B/: Driver function *populates metadata with offloads* from descriptor
...
XDP-prog creates map with special name e.g. "HW-offload"
- Map has BTF description
