...
** Slide: Compiling restricted-C to BPF into ELF                   :export:

/LLVM compiler/ has an BPF backend (to */avoid writing BPF assembly/* by hand)
...
- Need our *own ELF loader* that can:
  * Extract the BPF bytecode and BPF maps
  * Do ELF relocation of BPF maps references in bytecode
  * Create/load BPF maps and bytecode into kernel
