...
- How to get users to check features before attach? (bad solution)
...
** Current multi-program solutions #1: Katran xdp_root
...
** Current multi-program solutions #2: Cloudflare xdpdump
...
** How to implement? Option #2: Kernel support

With kernel support:

1. Define =bpf_chain_call()= as in-kernel helper
2. Special per-ifindex map managed by kernel
3. Verifier enforces rewrite of return codes
4. Userspace still responsible for populating chain call maps

/Pros/: Enforceable systemwide, verifier already rewrites progs

/*Cons*/: More kernel code, becomes API
