...
The reason for only enable XDP-TX queues when really needed is that this
...
Hope Toke have some ideas in this area.

Read abstract from: "BPF Host Network Resource Management"
 http://vger.kernel.org/lpc-networking.html#session-21
Sounds like they will somehow implement network shaping with BPF.
...
** Slide: Spectre-V2: mitigation killed XDP performance
