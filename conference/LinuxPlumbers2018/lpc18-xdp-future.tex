...
and userspace. The only difference between the modes is that for zero-copy mode,
userspace provides a chunk of pre-allocated memory that is used for raw frame
delivery. To support copy-mode the driver changes are minimal, as it only
requires supporting XDP\_REDIRECT. The zero-copy mode requires more driver
changes, as the userspace provides memory is used directly for DMA delivery.
