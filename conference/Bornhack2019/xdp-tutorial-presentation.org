...
Each eBPF runtime hook get a *pointer to a context* struct
- BPF-bytecode have access to context (read/write limited)
  * verifier step might adjust bytecode for safety

The BPF-program itself is stateless
- /Concept eBPF maps/ can be used to create state
- Maps are basically /key = value/ construct

BPF-helpers is a concept for
